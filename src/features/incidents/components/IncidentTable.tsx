...
function IncidentTable() {
  ...
  useEffect(() => {
    async function loadIncidents() {
      try {
        const data = await getIncidentRecords();
        setIncidents(data);
      } finally {
        setIsLoading(false);
      }
        ...
        <div className="divide-y divide-slate-800">
          {filteredIncidents.map((incident) => (
            <div
              ...
              className="grid gap-4 bg-slate-950/50 px-5 py-5 transition duration-300 hover:bg-slate-950/80 lg:grid-cols-[1fr_1fr_1fr_120px_140px_100px]"
            >
              <div>
