...
function OverviewPage() {
  ...
  useEffect(() => {
    async function loadOverviewData() {
      try {
        const [kpiData, trendData, distributionData, recentIncidentData] =
          await Promise.all([
            getOverviewKpis(),
            getIncidentTrend(),
            getCheatDistribution(),
    ...
    <>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_1fr]">
        ...
        >
          <div className="space-y-3">
            {[
              "Risk score spike detected for GhostByte",
              "Incident INC-2038 moved to Under Review",
              "Queue latency stable under burst load",
              "Detection rule triggered: abnormal recoil suppression",
              "Cloud health check passed across core modules",
            ].map((item, index) => (
              <div
                key={item}
